...
public interface BaoTriRepository extends JpaRepository<BaoTri, Long> {
    @Query("select new be.bds.bdsbes.payload.ChiTietPhongResponse1(p.id,p.tang,p.tienIch,p.dichVu,p.soLuongNguoi,p.dienTich, p.trangThai, ph.id, ph.ma) from ChiTietPhong p inner join p.phong ph")
